...
def _client_setup_updates(data: dict) -> list:
    ...
    # Program: exercises table (A15 downward: Type, Exercise, Sets, Reps, Notes, URL).
    ex_rows = [[e['workout_type'], e['exercise'], e['target_sets'],
                e['target_reps'], e['coach_notes'], e['tutorial_url']]
               for e in data['exercises']]
    if ex_rows:
        end = 15 + len(ex_rows) - 1
        updates.append({'range': f"'⚙ Program Builder'!A15:F{end}",
                        'values': ex_rows})

    # Week & RIR (rows 4 downward: week number, target RIR).
    rir_rows = [[w['week'], w['rir']] for w in data['weeks']]
    if rir_rows:
        end = 4 + len(rir_rows) - 1
